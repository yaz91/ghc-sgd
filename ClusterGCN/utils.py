...
def preprocess_multicluster(adj,
  ...
  for _, st in enumerate(range(0, num_clusters, block_size)):
    ...
    if diag_lambda == -1:
      ...
      support_batches.append(normalize_adj(support_now))
    else:
      # support_batches.append(
      #     sparse_to_tuple(normalize_adj_diag_enhance(support_now, diag_lambda)))
      support_batches.append(normalize_adj_diag_enhance(support_now, diag_lambda))
...
def load_data(data_prefix, dataset_str, precalc):
  ...
  (num_data, train_adj, full_adj, feats, train_feats, test_feats, labels,
   train_data, val_data,
   test_data) = load_graphsage_data(data_prefix, dataset_str)
